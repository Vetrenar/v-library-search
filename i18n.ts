...
export const TRANSLATIONS: Record<string, { en: string; ru: string }> = {
    ...
    // ── Settings: intro guide + top-level bands ───────────────────
    'settings.guideTitle':             { en: 'How it works', ru: 'Как это работает' },
    ...
    'settings.matchingDesc':           { en: 'What is searched, how it is grouped, and matching rules.', ru: 'Что ищется, как группируется и по каким правилам.' },
    'settings.presentation':           { en: 'Presentation',     ru: 'Отображение' },
    'settings.presentationDesc':       { en: 'Where and how results are shown.', ru: 'Где и как показываются результаты.' },
    ...
    'settings.groupPropertyDesc':      { en: 'Frontmatter key that decides membership, e.g. type.', ru: 'Ключ frontmatter, определяющий принадлежность к группе, напр. type.' },
    'settings.groupPropertyPh':        { en: 'frontmatter key (e.g. type)', ru: 'ключ frontmatter (напр. type)' },
    'settings.groupValues':            { en: 'Values',                  ru: 'Значения' },
    'settings.groupValuesDesc':        { en: 'Accepted values of the property, comma-separated. Empty = any non-empty value.', ru: 'Допустимые значения свойства через запятую. Пусто = любое непустое значение.' },
    'settings.groupValuesPh':          { en: 'values, comma-separated (empty = any)', ru: 'значения через запятую (пусто = любое)' },
    'settings.groupTermsSummary':      { en: 'Search terms',            ru: 'Термины поиска' },
    'settings.groupTermsDefault':      { en: 'Default (global source)', ru: 'По умолчанию (общий источник)' },
    'settings.groupTermsOwn':          { en: 'Custom term source for this group', ru: 'Свой источник терминов для этой группы' },
    'settings.groupTermsOwnDesc':      { en: 'Off — use the global source set above. On — choose a source just for this group.', ru: 'Выкл. — использовать общий источник выше. Вкл. — выбрать источник только для этой группы.' },
    ...
    'settings.callouts':               { en: 'Callouts',                ru: 'Выноски' },
    'settings.calloutTypes':           { en: 'Limit to callout types',  ru: 'Только типы выносок' },
    'settings.calloutTypesDesc':       { en: 'Comma-separated, e.g. note, quote, tip. Empty = any type.', ru: 'Через запятую, напр. note, quote, tip. Пусто = любой тип.' },
    'settings.lists':                  { en: 'List items & tasks',      ru: 'Пункты списков и задачи' },
    'settings.listsDesc':              { en: 'Search bullet/numbered list items. Slower on large notes.', ru: 'Искать в пунктах маркированных/нумерованных списков. На больших заметках медленнее.' },
    'settings.includeTasks':           { en: 'Include tasks (checkboxes)', ru: 'Включая задачи (чекбоксы)' },
    'settings.includeTasksDesc':       { en: 'Also match checkbox items like "- [ ] …".', ru: 'Также искать в пунктах-чекбоксах вида «- [ ] …».' },
    'settings.onlyUnchecked':          { en: 'Only unchecked tasks',    ru: 'Только невыполненные задачи' },
    'settings.onlyUncheckedDesc':      { en: 'Skip tasks that are already checked.', ru: 'Пропускать уже отмеченные задачи.' },
    'settings.perTargetNote':          { en: 'Heading output and snippets are now configured per group, inside each group\u2019s "Where to search".', ru: 'Режим заголовков и сниппеты теперь настраиваются для каждой группы внутри её блока «Где искать».' },
    ...
    'settings.panelScope':              { en: 'Where the panel appears', ru: 'Где показывать панель' },
    'settings.panelScopeDesc':          { en: 'In all notes (default), only in chosen folders, or everywhere except chosen folders.', ru: 'Во всех заметках (по умолчанию), только в выбранных папках или везде, кроме выбранных папок.' },
    'settings.scopeAll':                { en: 'All notes', ru: 'Везде' },
    'settings.scopeOnly':               { en: 'Only in these folders', ru: 'Только в этих папках' },
    'settings.scopeExcept':             { en: 'Everywhere except these folders', ru: 'Везде, кроме этих папок' },
    'settings.panelFolders':            { en: 'Folders', ru: 'Папки' },
    'settings.panelFoldersDesc':        { en: 'Folder paths, comma-separated.', ru: 'Пути папок через запятую.' },
